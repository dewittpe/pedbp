...
```{r }
qs <-
  unlist(subset(cdc_length_for_age, male == 1 & age == 144.5)[, grepl("p\\d+", names(cdc_length_for_age)) ])
...
```{r label = "eg_cdf_cap", echo = FALSE, results = "hide"}
cap <-
...
```{r label = "eg_cdf_cap2", echo = FALSE, results = "hide"}
cap <-
  paste0("Length for Age quantiles for 12-year old males with a Gausssian ",
         "cumulative distribution function overlaid.  The mean and standard deviations",
