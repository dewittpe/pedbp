...
test_df <-
  ...
  , p         = ps
  , z         = qnorm(ps)
  , q_cdc.who = suppressWarnings(q_weight_for_stature(p = ps, male = genders, age = ages, source = "CDC-WHO"))
  , q_cdc     = suppressWarnings(q_weight_for_stature(p = ps, male = genders, age = ages, source = "CDC"))
  , q_who     = suppressWarnings(q_weight_for_stature(p = ps, male = genders, age = ages, source = "WHO"))
  )

test_df[["p_cdc.who"]] <- suppressWarnings(p_weight_for_stature(q = test_df$q_cdc.who, male = genders, age = ages, source = "CDC-WHO"))
test_df[["p_cdc"]] <- suppressWarnings(p_weight_for_stature(q = test_df$q_cdc, male = genders, age = ages, source = "CDC"))
test_df[["p_who"]] <- suppressWarnings(p_weight_for_stature(q = test_df$q_who, male = genders, age = ages, source = "WHO"))

test_df[["z_cdc.who"]] <- suppressWarnings(z_weight_for_stature(q = test_df$q_cdc.who, male = genders, age = ages, source = "CDC-WHO"))
test_df[["z_cdc"]] <- suppressWarnings(z_weight_for_stature(q = test_df$q_cdc, male = genders, age = ages, source = "CDC"))
test_df[["z_who"]] <- suppressWarnings(z_weight_for_stature(q = test_df$q_who, male = genders, age = ages, source = "WHO"))
